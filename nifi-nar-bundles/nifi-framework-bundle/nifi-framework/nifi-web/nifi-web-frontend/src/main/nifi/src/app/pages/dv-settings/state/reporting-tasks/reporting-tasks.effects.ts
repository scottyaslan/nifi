    ...
    navigateToEditReportingTask$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                tap((id) => {
                    this.router.navigate(['/datavolo/settings', 'reporting-tasks', id, 'edit']);
                })
            ),
        { dispatch: false }
    );
            ...
            this.actions$.pipe(
                ...
                tap((request) => {
                    ...
                    editDialogReference.componentInstance.goToService = (serviceId: string) => {
                        const commands: string[] = ['/datavolo/settings', 'management-controller-services', serviceId];
    ...
    selectReportingTask$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                map((action) => action.request),
                tap((request) => {
                    this.router.navigate(['/datavolo/settings', 'reporting-tasks', request.id]);
                })
            ),
        { dispatch: false }
    );
