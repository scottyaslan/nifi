    ...
    function (nfCanvasUtils,
        ...
        var initializeSourceProcessor = function (source) {
            return $.Deferred(function (deferred) {
                ...
                $('#processor-source-type').text(nfCommon.substringAfterLast(processorData.component.type, '.'));
            ...
            return $.Deferred(function (deferred) {
                ...
                $.ajax({
                    ...
                    if (!nfCommon.isEmpty(processGroupContents.outputPorts)) {
                        ...
                        nfDialog.showOkDialog({
                            dialogContent: '\'' + nfCommon.escapeHtml(processGroup.name) + '\' does not have any output ports.'
                        });
        ...
        var initializeDestinationProcessor = function (destination) {
            return $.Deferred(function (deferred) {
                ...
                $('#processor-destination-type').text(nfCommon.substringAfterLast(processorData.component.type, '.'));
        ...
        var initializeDestinationProcessGroup = function (destination) {
            return $.Deferred(function (deferred) {
                ...
                $.ajax({
                    ...
                    // only proceed if there are output ports
                    if (!nfCommon.isEmpty(processGroupContents.inputPorts)) {
                        ...
                        // there are no relationships for this processor
                        nfDialog.showOkDialog({
                            dialogContent: '\'' + nfCommon.escapeHtml(processGroup.name) + '\' does not have any input ports.'
                        });
        ...
        var addConnection = function (selectedRelationships) {
            ...
            if (validateSettings()) {
                ...
                $.ajax({
                    type: 'POST',
                    url: config.urls.controller + '/process-groups/' + encodeURIComponent(nfCanvas.getGroupId()) + '/connections',
                    data: {
        ...
        var updateConnection = function (selectedRelationships) {
            ...
            if (validateSettings()) {
                ...
                return $.ajax({
                    ...
                    if (nfCommon.isDefinedAndNotNull(response.connection)) {
            ...
            init: function () {
                ...
                $.ajax({
                    ...
                    $.each(response.prioritizerTypes, function (i, documentedType) {
                        self.addAvailablePrioritizer('#prioritizer-available', documentedType);
                    });
