...
/* global nf, d3, define, module, require, exports */
...
(function (root, factory) {
    ...
    var renderProcessGroups = function (entered, selected) {
        ...
        processGroup.on('dblclick', function (d) {
            ...
            .call(nfSelectable.activate).call(nfContextMenu.activate, connection);
    ...
    var nfProcessGroup = {
        ...
        enterGroup: function (groupId) {  // hide the context menu
            ...
            nfContextMenu.hide();   // set the new group id

            nfCanvas.setGroupId(groupId);   // reload the graph

            return nfCanvas.reload().done(function () {  // attempt to restore the view

                var viewRestored = canvasUtils.restoreUserView();   // if the view was not restore attempt to fit

                if (viewRestored === false) {
                    nfCanvas.View.fit();   // refresh the canvas
