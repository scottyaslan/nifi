...
//     $('img.setting-icon').qtip(this.config.tooltipConfig);
//    
    ...
    function (nfStorage,
        ...
        return {
            ...
            scheduleTokenRefresh: function () {
                ...
                var interval = this.MILLIS_PER_MINUTE;
                ...
                var checkExpiration = function () {
                    ...
                    if (expiration !== null) {
                        ...
                        var timeRemaining = expirationDate.valueOf() - now.valueOf() - (30 * this.MILLIS_PER_SECOND);
                        if (timeRemaining < interval) {
                            if ($('#current-user').text() !== this.ANONYMOUS_USER_TEXT && !$('#anonymous-user-alert').is(':visible')) {
                                // if the token will expire before the next interval minus some bonus time, notify the user to re-login
                                $('#anonymous-user-alert').show().qtip($.extend({}, this.config.tooltipConfig, {
            ...
            setAnonymousUserLabel: function () {
                ...
                anonymousUserAlert.show().qtip($.extend({}, this.config.tooltipConfig, {
                ...
                $('#current-user').text(this.ANONYMOUS_USER_TEXT).show();
            },
            ...
            getJwtPayload: function (jwt) {
                if (this.isDefinedAndNotNull(jwt)) {
                    ...
                    if (this.isDefinedAndNotNull(payload)) {
                        return payload;
                    } else {
                        return null;
                    }
            ...
            canAccessProvenance: function () {
                ...
                if (this.isDefinedAndNotNull(this.authorities)) {
                    $.each(this.authorities, function (i, authority) {
            ...
            handleAjaxError: function (xhr, status, error) {
                ...
                if (xhr.status === 400 || xhr.status === 404 || xhr.status === 409) {
                    nfDialog.showOkDialog({
                        dialogContent: this.escapeHtml(xhr.responseText),
                        overlayBackground: false
                    });
                } else {
                    if (xhr.status < 99 || xhr.status === 12007 || xhr.status === 12029) {
                        ...
                        if (this.isDefinedAndNotNull(status)) {
                            if (status === 'timeout') {
            ...
            isContentViewConfigured: function () {
                ...
                return !this.isBlank(contentViewerUrl);
            },
            ...
            formatPropertyTooltip: function (propertyDescriptor, propertyHistory) {
                ...
                if (this.isDefinedAndNotNull(propertyDescriptor)) {
                    if (!this.isBlank(propertyDescriptor.description)) {
                        tipContent.push(this.escapeHtml(propertyDescriptor.description));
                    }
                    if (!this.isBlank(propertyDescriptor.defaultValue)) {
                        tipContent.push('<b>Default value:</b> ' + this.escapeHtml(propertyDescriptor.defaultValue));
                    }
                    if (!this.isBlank(propertyDescriptor.supportsEl)) {
                        tipContent.push('<b>Supports expression language:</b> ' + this.escapeHtml(propertyDescriptor.supportsEl));
                    }
                }

                if (this.isDefinedAndNotNull(propertyHistory)) {
                    if (!this.isEmpty(propertyHistory.previousValues)) {
                        var history = [];
                        $.each(propertyHistory.previousValues, function (_, previousValue) {
                            history.push('<li>' + this.escapeHtml(previousValue.previousValue) + ' - ' + this.escapeHtml(previousValue.timestamp) + ' (' + this.escapeHtml(previousValue.userName) + ')</li>');
                        });
            ...
            formatProperty: function (name, value) {
                return '<div><span class="label">' + this.formatValue(name) + ': </span>' + this.formatValue(value) + '</div>';
            },
            ...
            escapeHtml: (function () {
                ...
                return function (string) {
                    if (this.isDefinedAndNotNull(string)) {
            ...
            post: function (url, params) {
                ...
                if (this.isDefinedAndNotNull(params)) {
            ...
            formatDataSize: function (dataSize) {
                // check terabytes
                var dataSizeToFormat = parseFloat(dataSize / this.BYTES_IN_TERABYTE);
                if (dataSizeToFormat > 1) {
                    return dataSizeToFormat.toFixed(2) + " TB";
                }

                // check gigabytes
                dataSizeToFormat = parseFloat(dataSize / this.BYTES_IN_GIGABYTE);
                if (dataSizeToFormat > 1) {
                    return dataSizeToFormat.toFixed(2) + " GB";
                }

                // check megabytes
                dataSizeToFormat = parseFloat(dataSize / this.BYTES_IN_MEGABYTE);
                if (dataSizeToFormat > 1) {
                    return dataSizeToFormat.toFixed(2) + " MB";
                }

                // check kilobytes
                dataSizeToFormat = parseFloat(dataSize / this.BYTES_IN_KILOBYTE);
                if (dataSizeToFormat > 1) {
            ...
            parseDateTime: function (rawDateTime) {
                ...
                if (!this.isDefinedAndNotNull(rawDateTime)) {
                    return new Date();
                }
            ...
            isBlank: function (str) {
                return this.isUndefined(str) || this.isNull(str) || $.trim(str) === '';
            },
            ...
            getFormattedBulletins: function (bulletins) {
                ...
                $.each(bulletins, function (j, bulletin) {
                    ...
                    var nodeAddress = '';
                    if (this.isDefinedAndNotNull(bulletin.nodeAddress)) {
                        nodeAddress = '-&nbsp' + this.escapeHtml(bulletin.nodeAddress) + '&nbsp;-&nbsp;';
                    }
                    ...
                    var formattedBulletin = $('<div>' +
                        this.escapeHtml(bulletin.timestamp) + '&nbsp;' +
                        nodeAddress + '&nbsp;' +
                        '<b>' + this.escapeHtml(bulletin.level) + '</b>&nbsp;' +
