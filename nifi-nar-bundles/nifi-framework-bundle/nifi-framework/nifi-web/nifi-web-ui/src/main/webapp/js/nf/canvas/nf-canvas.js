...
define(['nf-common',
    ...
    function (nfCommon,
              ...
              nfControllerService,
              nfReportingTask,
              nfProcessorConfiguration,
              nfProcessGroupConfiguration,
              nfRemoteProcessGroupConfiguration,
              nfRemoteProcessGroupPorts,
              nfPortConfiguration,
              nfSecurePortConfiguration,
              nfLabelConfiguration,
              nfProcessorDetails,
              nfProcessGroupDetails,
              nfPortDetails,
              nfSecurePortDetails,
              nfConnectionDetails,
              nfRemoteProcessGroupDetails,
              nfGoTo,
              nfCanvasView,
              nfCanvasToolbox,
              nfCanvasToolbar,
        ...
        var generateBreadcrumbs = function (processGroup) {
            ...
            var groupLink = $('<span class="link"></span>').text(processGroup.name).click(function () {
                thisUtils.enterGroup(processGroup.id);
            });

            // make the current group bold
            if (this.getGroupId() === processGroup.id) {
                groupLink.css('font-weight', 'bold');
            }

            // if there is a parent, create the appropriate mark up
            if (nfCommon.isDefinedAndNotNull(processGroup.parent)) {
         ...
         */
        var loadD3 = function () {
            return nfCommon.cachedScript(config.urls.d3Script);
        };
        ...
        var pollForStatus = function (autoRefreshInterval) {
            ...
            if (statusPolling) {
                // reload the status
                this.reloadStatus().done(function () {
                    // start the wait to poll again
                    setTimeout(function () {
                        pollForStatus(autoRefreshInterval);
                    }, autoRefreshInterval * 1000);
        ...
        var checkRevision = function () {
            ...
            return $.ajax({
                ...
                if (nfCommon.isDefinedAndNotNull(response.revision)) {
                    var revision = response.revision;
                    var currentRevision = nfClient.getRevision();
                    ...
                    if (revision.version > currentRevision.version && revision.clientId !== currentRevision.clientId) {
                        ...
                        if (!refreshContainer.is(':visible')) {
                            ...
                            if (refreshRequiredIcon.data('qtip')) {
                                ...
                                }, thisUtils.config.systemTooltipConfig));
                            }
        ...
        var initCanvas = function () {
            ...
            svg = d3.select('#canvas-container').append('svg')
                .on('contextmenu', function () {
                    ...
                    thisUtils.getSelection().classed('selected', false);
                    ...
                    if (d3.event.shiftKey) {
                        ...
                        canvas.append('rect')
                            ...
                            .attr('stroke-width', function () {
                                return 1 / nfCanvasView.scale();
                            })
                            .attr('stroke-dasharray', function () {
                                return 4 / nfCanvasView.scale();
                            })
                            .datum(position);
            ...
            $(window).on('resize', function (e) {
                if (e.target === window) {
                    updateGraphSize();
                    nfSettings.resetTableSize(this);
                }
                ...
                var selection = thisUtils.getSelection();
                ...
                if (isCtrl) {
                    if (evt.keyCode === 82) {
                        ...
                        // ctrl-c
                        if (nfCommon.isDFM() && thisUtils.isCopyable(selection)) {
                        ...
                        // ctrl-v
                        if (nfCommon.isDFM() && thisUtils.isPastable()) {
                    ...
                    if (evt.keyCode == 8 || evt.keyCode === 46) {
                        // backspace or delete
                        if (nfCommon.isDFM() && thisUtils.isDeletable(selection)) {
        ...
        var reloadFlowStatus = function () {
            return $.ajax({
                ...
                if (nfCommon.isDefinedAndNotNull(response.controllerStatus)) {
                    ...
                    if (nfCommon.isDefinedAndNotNull(controllerStatus.connectedNodes)) {
                    ...
                    if (nfCommon.doBulletinsDiffer(currentBulletins, controllerStatus.bulletins)) {
                        bulletinIcon.data('bulletins', controllerStatus.bulletins);

                        // get the formatted the bulletins
                        var bulletins = nfCommon.getFormattedBulletins(controllerStatus.bulletins);
                    ...
                    nfSettings.setBulletins(controllerStatus.controllerServiceBulletins, controllerStatus.reportingTaskBulletins, this);
        ...
        var reloadProcessGroup = function (processGroupId) {
            ...
            return $.ajax({
                ...
                // set the revision
                nfClient.setRevision(processGroupResponse.revision);

                // get the controller and its contents
                var processGroup = processGroupResponse.processGroup;

                // set the group details
                this.setGroupId(processGroup.id);
                this.setGroupName(processGroup.name);
                ...
                if (nfCommon.isDefinedAndNotNull(processGroup.parent)) {
                    this.setParentGroupId(processGroup.parent.id);
                } else {
                    this.setParentGroupId(null);
                }
                ...
                // refresh the graph
                nfGraph.add(processGroup.contents, false);
            ...
            return $.Deferred(function (deferred) {
                $.ajax({
                    ...
                    if (nfCommon.isDefinedAndNotNull(response.processGroupStatus)) {
                        ...
                        nfGraph.setStatus(aggregateSnapshot);
                    ...
                    if (!this.isClustered() || xhr.status !== 404) {
        ...
        return {
            ...
            closeCanvas: function () {
                ...
                if (this.isDefinedAndNotNull(nfCanvas)) {
                    ...
                    nfCanvas.stopRevisionPolling();
                    nfCanvas.stopStatusPolling();
                }
            },

            /**
             * Reloads the flow from the server based on the currently specified group id.
             * To load another group, update this.setGroupId and call this.reload.
             */
            reload: function () {
                return $.Deferred(function (deferred) {
                    // hide the context menu
                    ...
                    var processGroupXhr = reloadProcessGroup(this.getGroupId());
                    var statusXhr = reloadFlowStatus();
                    var settingsXhr = nfSettings.loadSettings(false, this); // don't reload the status as we want to wait for deferreds to complete
                    $.when(processGroupXhr, statusXhr, settingsXhr).done(function (processGroupResult) {
                        ...
                        reloadStatus(this.getGroupId()).done(function () {
            ...
            reloadStatus: function () {
                return $.Deferred(function (deferred) {
                    ...
                    $.when(reloadStatus(this.getGroupId()), reloadFlowStatus(), checkRevision()).done(function () {
                        deferred.resolve();
                    }).fail(function () {
                        deferred.reject();
                    });
            ...
            init: function (storage) {
                ...
                var userXhr = $.Deferred(function (deferred) {
                    ticketExchange.always(function () {
                        ...
                        $.when(authoritiesXhr, identityXhr).done(function (authoritiesResult, identityResult) {
                            ...
                            nfCommon.setAuthorities(authoritiesResponse.authorities);
                ...
                userXhr.done(function () {
                    ...
                    $.when(configXhr, loginXhr, userXhr).done(function (configResult, loginResult) {
                        ...
                        var canvasContainer = $('#canvas-container');
                        this.CANVAS_OFFSET = canvasContainer.offset().top;
                        ...
                        if (!nfCommon.isBlank(configDetails.contentViewerUrl)) {
                        ...
                        isClusteredRequest.done(function () {
                            ...
                            loadD3().done(function () {
                                ...
                                initCanvas();
                                nfCanvasView.init(MIN_SCALE, MAX_SCALE, TRANSLATE, SCALE, svg);
                                nfContextMenu.init();
                                nfCanvasToolbar.init();
                                nfCanvasToolbox.init();
                                nfCanvasHeader.init(loginDetails.supportsLogin, self);
                                nfGraphControl.init();
                                nfSearch.init();
                                nfSettings.init(self);
                                nfActions.init();
                                nfQueueListing.init(self);
                                nfComponentState.init(self);
                                ...
                                nfConnectionConfiguration.init();
                                nfControllerService.init(self);
                                nfReportingTask.init(self);
                                nfProcessorConfiguration.init();
                                nfProcessGroupConfiguration.init();
                                nfRemoteProcessGroupConfiguration.init();
                                nfRemoteProcessGroupPorts.init();
                                nfPortConfiguration.init();
                                nfSecurePortConfiguration.init();
                                nfLabelConfiguration.init();
                                nfProcessorDetails.init();
                                nfProcessGroupDetails.init();
                                nfPortDetails.init();
                                nfSecurePortDetails.init();
