...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jquery',
                ...
                'nf.Actions',
                'nf.ProcessGroup'],
            function ($, d3, common, graph, shell, angularBridge, clusterSummary, errorHandler, storage, canvasUtils, birdseye, contextMenu, actions, processGroup) {
                return (nf.Canvas = factory($, d3, common, graph, shell, angularBridge, clusterSummary, errorHandler, storage, canvasUtils, birdseye, contextMenu, actions, processGroup));
            });
...
}(this, function ($, d3, common, graph, shell, angularBridge, clusterSummary, errorHandler, storage, canvasUtils, birdseye, contextMenu, actions, processGroup) {
    'use strict';
    ...
    var reloadProcessGroup = function (processGroupId, options) {
        ...
        return $.ajax({
            ...
            // refresh the graph
            graph.expireCaches(now);
            graph.set(processGroupFlow.flow, $.extend({
                'selectAll': false
            }, options));
    ...
    return {
        ...
        reload: function (options) {
            return $.Deferred(function (deferred) {
                ...
                var clusterSummary = clusterSummary.loadClusterSummary().done(function (response) {
                ...
                if (e.target === window) {
                    ...
                    if (shell.is(':visible')){
                        setTimeout(function(shell){
                            shell.resizeContent(shell);
                            if(shell.find('#shell-iframe').is(':visible')) {
                                shell.resizeIframe(shell);
                            }
                        }, 50, shell);
                    }
