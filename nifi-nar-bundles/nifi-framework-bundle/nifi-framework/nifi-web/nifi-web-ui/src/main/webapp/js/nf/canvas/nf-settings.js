...
define(['nf-client',
    ...
    function (nfClient,
         ...
         */
        var addControllerService = function (controllerServiceType, canvas) {
            var revision = nfClient.getRevision();

            // get the desired availability
            var availability;
            if (canvas.isClustered()) {
         ...
         */
        var initNewControllerServiceDialog = function (canvas) {
        ...
        var typeFormatter = function (row, cell, value, columnDef, dataContext) {
            return nfCommon.substringAfterLast(value, '.');
        };
        ...
        var sort = function (sortDetails, data) {
            ...
            var comparer = function (a, b) {
                if (sortDetails.columnId === 'moreDetails') {
                    ...
                    var aType = nfCommon.isDefinedAndNotNull(a[sortDetails.columnId]) ? nfCommon.substringAfterLast(a[sortDetails.columnId], '.') : '';
                    var bType = nfCommon.isDefinedAndNotNull(b[sortDetails.columnId]) ? nfCommon.substringAfterLast(b[sortDetails.columnId], '.') : '';
                    return aType === bType ? 0 : aType > bType ? 1 : -1;
                } else if (sortDetails.columnId === 'state') {
                    var aState = 'Invalid';
                    if (nfCommon.isEmpty(a.validationErrors)) {
                        aState = nfCommon.isDefinedAndNotNull(a[sortDetails.columnId]) ? a[sortDetails.columnId] : '';
                    }
                    var bState = 'Invalid';
                    if (nfCommon.isEmpty(b.validationErrors)) {
                        bState = nfCommon.isDefinedAndNotNull(b[sortDetails.columnId]) ? b[sortDetails.columnId] : '';
                    }
         ...
         */
        var initControllerServices = function (canvas) {
            // initialize the new controller service dialog
            initNewControllerServiceDialog(canvas);
            ...
            var moreControllerServiceDetails = function (row, cell, value, columnDef, dataContext) {
                ...
                var hasErrors = !nfCommon.isEmpty(dataContext.validationErrors);
            ...
            var controllerServiceStateFormatter = function (row, cell, value, columnDef, dataContext, canvas) {
                // determine the appropriate label
                var icon = '', label = '';
                if (!nfCommon.isEmpty(dataContext.validationErrors)) {
                    icon = 'invalid';
                    label = 'Invalid';
                } else {
                    if (value === 'DISABLED') {
            ...
            controllerServicesGrid.onClick.subscribe(function (e, args) {
                ...
                if (controllerServicesGrid.getColumns()[args.cell].id === 'actions') {
                    if (target.hasClass('edit-controller-service')) {
                        nfControllerService.showConfiguration(controllerService);
                    } else if (target.hasClass('enable-controller-service')) {
                        nfControllerService.enable(controllerService);
                    } else if (target.hasClass('disable-controller-service')) {
                        nfControllerService.disable(controllerService);
                    } else if (target.hasClass('delete-controller-service')) {
                        nfControllerService.remove(controllerService);
                    } else if (target.hasClass('view-state-controller-service')) {
                        nfComponentState.showState(controllerService, controllerService.state === 'DISABLED');
                    }
                } else if (controllerServicesGrid.getColumns()[args.cell].id === 'moreDetails') {
                    if (target.hasClass('view-controller-service')) {
                        nfControllerService.showDetails(controllerService);
                        ...
                        nfShell.showPage('../nifi-docs/documentation?' + $.param({
                                select: nfCommon.substringAfterLast(controllerService.type, '.')
                            })).done(function() {
                            this.showSettings();
                        });
                    }
            ...
            $('#controller-services-table').data('gridInstance', controllerServicesGrid).on('mouseenter', 'div.slick-cell', function (e) {
                ...
                if (errorIcon.length && !errorIcon.data('qtip')) {
                    ...
                    // format the errors
                    var tooltip = nfCommon.formatUnorderedList(item.validationErrors);
         ...
         */
        var loadControllerServices = function (canvas) {
            var services = [];
            ...
            var nodeControllerServices = $.ajax({
                ...
                if (nfCommon.isDefinedAndNotNull(nodeServices)) {
            ...
            var ncmControllerServices = $.Deferred(function (deferred) {
                if (canvas.isClustered()) {
                    $.ajax({
                        ...
                        if (nfCommon.isDefinedAndNotNull(ncmServices)) {
            ...
            return $.when(nodeControllerServices, ncmControllerServices).done(function () {
                ...
                nfCommon.cleanUpTooltips(controllerServicesElement, 'img.has-errors');
                nfCommon.cleanUpTooltips(controllerServicesElement, 'img.has-bulletins');
         ...
         */
        var addReportingTask = function (reportingTaskType, canvas) {
            var revision = nfClient.getRevision();
         ...
         */
        var initNewReportingTaskDialog = function (canvas) {
         ...
         */
        var initReportingTasks = function (canvas) {
            // initialize the new reporting task dialog
            initNewReportingTaskDialog(canvas);
            ...
            var reportingTaskRunStatusFormatter = function (row, cell, value, columnDef, dataContext) {
                ...
                // format the markup
                var formattedValue = '<div class="' + nfCommon.escapeHtml(label.toLowerCase()) + '" style="margin-top: 3px;"></div>';
                return formattedValue + '<div class="status-text" style="margin-top: 2px; margin-left: 4px; float: left;">' + nfCommon.escapeHtml(label) + '</div><div style="float: left; margin-left: 4px;">' + nfCommon.escapeHtml(activeThreadCount) + '</div>';
            };

            var reportingTaskActionFormatter = function (row, cell, value, columnDef, dataContext, canvas) {
                var markup = '';
                ...
                if (nfCommon.isDFM()) {
                    if (dataContext.state === 'RUNNING') {
                        ...
                        if (dataContext.state === 'STOPPED' && nfCommon.isEmpty(dataContext.validationErrors)) {
            ...
            reportingTasksGrid.onClick.subscribe(function (e, args) {
                ...
                if (reportingTasksGrid.getColumns()[args.cell].id === 'actions') {
                    if (target.hasClass('edit-reporting-task')) {
                        nfReportingTask.showConfiguration(reportingTask);
                    } else if (target.hasClass('start-reporting-task')) {
                        nfReportingTask.start(reportingTask);
                    } else if (target.hasClass('stop-reporting-task')) {
                        nfReportingTask.stop(reportingTask);
                    } else if (target.hasClass('delete-reporting-task')) {
                        nfReportingTask.remove(reportingTask);
                    ...
                    if (target.hasClass('view-reporting-task')) {
                        ...
                        nfShell.showPage('../nifi-docs/documentation?' + $.param({
                                select: nfCommon.substringAfterLast(reportingTask.type, '.')
                            })).done(function() {
                            this.showSettings();
                        });
                    }
         ...
         */
        var loadReportingTasks = function (canvas) {
            var tasks = [];
            ...
            var nodeReportingTasks = $.ajax({
                ...
                if (nfCommon.isDefinedAndNotNull(nodeTasks)) {
            ...
            var ncmReportingTasks = $.Deferred(function (deferred) {
                if (canvas.isClustered()) {
                    $.ajax({
                        ...
                        if (nfCommon.isDefinedAndNotNull(ncmTasks)) {
            ...
            return $.when(nodeReportingTasks, ncmReportingTasks).done(function () {
                ...
                nfCommon.cleanUpTooltips(reportingTasksElement, 'img.has-errors');
                nfCommon.cleanUpTooltips(reportingTasksElement, 'img.has-bulletins');
        ...
        return {
            ...
            init: function (canvas) {
                ...
                nfCommon.addHoverEffect('#settings-refresh-button', 'button-refresh', 'button-refresh-hover').click(function () {
                    if ($('#settings-refresh-required-icon').is(':visible')) {
                        nfCanvasHeader.reloadAndClearWarnings();
                    } else {
                        this.loadSettings();
                    }
                });
                ...
                // initialize each tab
                initGeneral();
                initControllerServices(canvas);
                initReportingTasks(canvas);
            },
             ...
             * Loads the settings.
             */
            loadSettings: function (reloadStatus, canvas) {
                var settings = $.ajax({
                    type: 'GET',
                    url: config.urls.controllerConfig,
                    dataType: 'json'
                    ...
                    if (nfCommon.isDefinedAndNotNull(response.config)) {
                        // set the header
                        ...
                        if (nfCommon.isDFM()) {
                            ...
                            $('#read-only-data-flow-title-field').html(nfCommon.formatValue(response.config.name));
                            $('#read-only-data-flow-comments-field').html(nfCommon.formatValue(response.config.comments));
                ...
                var controllerServices = loadControllerServices(canvas);

                // load the reporting tasks
                var reportingTasks = loadReportingTasks(canvas);
