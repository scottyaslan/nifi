...
define(['nf-common',
    ...
    function (nfCommon,
        ...
        var isSaveRequired = function () {
            ...
            if (nfCommon.isDefinedAndNotNull(schedulingPeriod) && schedulingPeriod.val() !== (details['schedulingPeriod'] + '')) {
                return true;
            }
        ...
        var marshalDetails = function () {
            ...
            reportingTaskEntity['revision'] = nfClient.getRevision();
        ...
        var setRunning = function (reportingTask, running) {
            ...
            return $.ajax({
                ...
                nfControllerService.reloadReferencedServices(response.reportingTask);
        ...
        var saveReportingTask = function (reportingTask) {
            ...
            if (validateDetails(updatedReportingTask)) {
                ...
                return $.ajax({
                    ...
                    if (nfCommon.isDefinedAndNotNull(response.reportingTask)) {
                        // update the revision
                        nfClient.setRevision(response.revision);
                    }
        ...
        return {
             ...
             */
            showConfiguration: function (reportingTask, canvas) {
                            ...
                            handler: {
                                click: function () {
                                    var openCustomUi = function () {
                                        ...
                                        // show the custom ui
                                        nfCustomUi.showCustomUi($('#reporting-task-id').text(), reportingTask.customUiUrl, true).done(function () {
                                            // once the custom ui is closed, reload the reporting task
                                            this.reload(reportingTask.id).done(function (response) {
                                                nfControllerService.reloadReferencedServices(response.reportingTask);
                                            });
             ...
             * @param {object} reportingTask
             */
            showDetails: function(reportingTask, canvas) {
                var reportingTaskDialog = $('#reporting-task-configuration');
                        ...
                        buttons.push({
                            ...
                            handler: {
                                click: function () {
                                    ...
                                    // show the custom ui
                                    nfCustomUi.showCustomUi(reportingTask.id, reportingTask.customUiUrl, false).done(function() {
                                        nfSettings.showSettings();
                                    });
                                }
                            }
